function [resultado,entropy,racio] = block_compression_txt_B(input, encode_or_d, kBytes) %computaçao em blocos parametrizavel
...
%   No máximo podemos representar 255*255 = 65025, pelo que o limite é de 65kBytes por bloco
    ...
    if encode_or_d == 0 %encoding
        ...
        for i=1:bytes:len
            ...
            compressed_block = delta_encoding(compressed_block,0);
            compressed_block = rle(compressed_block,0);
            compressed_block = cat(2,bwt_index,compressed_block);
            
            entropy = entropy + entropia(compressed_block);
            %COMPRESSÃO DO BLOCO -----------------------
        ...
        entropy = entropy / numBlocos;
        racio = ( length(input)-1 ) / length(resultado);
    ...
    elseif encode_or_d == 1 %decoding
        ...
        while processed_bytes < len
            ...
            original_block = rle(compressed_block,1);
            original_block = delta_encoding(original_block,1);
