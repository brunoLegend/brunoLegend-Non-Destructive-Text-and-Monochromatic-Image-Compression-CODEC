function [resultado,racio] = codificacao_img_W(input, encode_or_d, kBytes) %compressao em blocos parametrizavel
...
%   No máximo podemos representar 255*255 = 65025, pelo que o limite é de 65kBytes por bloco
    ...
    if encode_or_d == 0 %encoding
        ...
        for i=1:bytes:len
            ...
            extra = extra + 2 + length(info.huffcodes) + length(info.length) + length(Counts) + length(Table); 
        ...
        racio = ( length(input)-3 ) / ( length(resultado) + extra );
