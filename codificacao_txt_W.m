function [resultado,racio] = codificacao_txt_W(input, encode_or_d, kBytes) %compressao em blocos parametrizavel
...
%   No máximo podemos representar 255*255 = 65025, pelo que o limite é de 65kBytes por bloco
    ...
    if encode_or_d == 0 %encoding
        ...
        for i=1:bytes:len
            ...
            compressed_block = norm2lzw(compressed_block,Inf);
            
            [compressed_block,info] = norm2huff(compressed_block);
            extra = extra + 2 + length(info.huffcodes) + length(info.length); 
    ...
    elseif encode_or_d == 1 %decoding
        ...
        while processed_bytes < len
            ...
            original_block = lzw2norm(compressed_block,Inf);
